...
# Build
build() {
    if command -v warp &> /dev/null || command -v vpn &> /dev/null; then
...
socks() {
   ...
   echo "================================================"
   echo -e "${green}socks://Og==@127.0.0.1:8086#warp_(vpn)${rest}"
   echo "or"
   echo -e "${green}Manually create a SOCKS configuration with IP ${purple}127.0.0.1 ${green}and port${purple} 8086..${rest}"
   echo "================================================"
   echo -e "${yellow}To run again, type:${green} warp ${rest}or${green} vpn ${rest}or${green} ./warp${rest}"
   echo "================================================"
...
uninstall() {
    ...
    if [ -f "$warp" ]; then
        rm -rf "$directory" "$PREFIX/bin/vpn" "wa.py" "$PREFIX/bin/warp" "$home/wgcf-profile.ini" "$home/warp" "$home/wgcf-identity.json" > /dev/null 2>&1
